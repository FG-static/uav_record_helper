...
rh record   # 分阶段引导录制：预热 → 平放静止 → 温和激励 → 录制（手动停） → 尾部覆盖 → 出盘 → 自检
...
| 几何错误 | 三个 `T_BS` 旋转都是单位阵；D435i 的光学系相对 IMU 系是约 90° 的轴向置换，恒等外参会让重力方向和图像投影同时错 |
...
| 静止标定 | 终端门控，`--still` 秒（默认 4） | 平放桌上完全不动。不能跳 |
